...
from cobweb.datasets import load_mushroom
from RSA_node import RSA_CobwebNode_classification
...
preserved_attrs = ['cap-color', 'cap-shape', 'cap-surface', 'classification']
...
def _display_node(node, level):
	...
	level: int
		The depth of node.
	"""
	rsa_node = RSA_CobwebNode_classification(node, include_classification=False, level=level)
	# rsa_node = RSA_node(node=node, task='class')
